...
while True:
	...
	# Get the largest detected part, draw a rectangle
	if len(facerect) > 0:
		print("face detected!")
		face_height = facerect[:,3]
		index_max = numpy.argmax(face_height)
		rect = facerect[index_max]
		img_face = frame[rect[1]:rect[1]+rect[3], rect[0]:rect[0]+rect[2]]
		cv2.rectangle(frame, tuple(rect[0:2]), tuple(rect[0:2]+rect[2:4]), (0, 255, 0), thickness=8)
